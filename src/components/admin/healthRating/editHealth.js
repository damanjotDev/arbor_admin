...
import { editHealthRate } from '../../../API';
import { editHealthRateData } from '../../../actions/healthRating';
...
const EditHealth = () => {
  const dispatch = useDispatch()
   ...
   const location = useLocation()
    ...
    const {healthRate,loader}=useSelector(({healthRate})=>healthRate)

    const [healthData, setHealthData] = useState({
      rating:healthRate?.rating?healthRate?.rating:""
    })
    ...
    const handleSubmit =(e)=>{
      ...
      if(!healthData.rating){
        ...
        dispatch(editHealthRateData(healthData, navigate, location))
      }
    }
